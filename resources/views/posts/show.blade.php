        ...
        <div class="flex w-full flex-col bg-white md:w-5/12">
            ...
            <div class="border-b-2">
                ...
                <div class="flex items-center p-5 gap-0">
                    ...
                    @if ($post->user_id === Auth::id())
                        <a href="{{ route('posts.edit', $post->slug) }}"><i class='bx bx-edit'></i></a>
                        <form id="delete-post-form-{{ $post->id }}"
                            action="{{ route('posts.destroy', $post->slug) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="text-red-600" onclick="confirmDelete({{ $post->id }})">
                                <i class='bx bx-trash inline ml-2'></i>
                            </button>
                        </form>
                    @endif
                </div>
            ...
            <div class="grow">
                <div class="flex items-start px-5 py-2 gap-3">
                    <img src="{{ asset('storage/' . $post->owner->image) }}" alt="{{ $post->owner->username }}"
                        class="ltr:mr-5 rtl:m-5 h-10 w-10 rounded-full">
                ...
                @foreach ($post->comments as $comment)
                    <div class="flex items-start px-5 py-2 gap-2">
                        <img src="{{ asset('storage/' . $comment->owner->image) }}"
                            class="h-10 w-10 rounded-full ltr:mr-5 rtl:ml-5" alt="">
                        <div class="flex flex-col">
                            <div>
                                <a href=""
